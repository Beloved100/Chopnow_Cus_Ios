...
FetchHook useFetchRiderUser(String id) {
  final rider = useState<RiderUserModel?>(null); 
  ...
  Future<void> fetchData() async {
    ...
    try {
      final url = Uri.parse("$appBaseUrl/api/rider/user/$id");
      final response = await http.get(url);
      print("useFetchRider: ${response.body}");

      if (response.statusCode == 200) {
        var user = jsonDecode(response.body);
        rider.value = RiderUserModel.fromJson(user);
        print("useFetchRider: ${response.statusCode}");
      } else {
